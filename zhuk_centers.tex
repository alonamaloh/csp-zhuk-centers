...
\item If $C=\varnothing$ then the absorption assertions about $C$ are vacuous
      ...
      elements of $C$; see Remark~\ref{rem:induction-shape}. The one case in
      which the prescribed witness $t^{*(|B|-1)}$ does have arity $1$ is
      $|B|=1$, and there subdirectness gives $C=A\ne\varnothing$, so the
      assertion is not vacuous but true.
...
Lemma~\ref{lem:center-basic}(c) and Lemma~\ref{lem:preservation}. Hence
Theorem~\ref{thm:center-step}, applied with $d_j$ in position $j$ and the
$C$-elements $d_{j'}$ elsewhere, gives
\[
...
inferential. Second, several rows therefore cite forward --- the remarks
\ref{rem:fixed-list}, \ref{rem:substitution-usage},
\ref{rem:absorption-quantifier}, \ref{rem:taylor-usual} and
\ref{rem:pp-usage} all point ahead to material they motivate. The only
\emph{proof} dependency that runs forward is that of Theorem~\ref{thm:main},
which is stated at the front and proved at the end.
...
Remark~\ref{rem:degenerate-center}\newline\emph{Degenerate centers} & Remark~\ref{rem:absorption-quantifier}, Theorem~\ref{thm:center-absorbs}, Definition~\ref{def:absorption}, Remark~\ref{rem:induction-shape}, Theorem~\ref{thm:center-central} and Lemma~\ref{lem:generation} \\
