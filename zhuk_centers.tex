...
\begin{lemma}[Grouping and enumeration]\label{lem:grouping}
Let $\bA$ be an algebra, let $I$ be a finite set, and let
$I=I_1\sqcup\cdots\sqcup I_m$ be a partition into nonempty blocks. Put
$n_j:=|I_j|$, $n:=n_1+\cdots+n_m$, and let $\beta_j\subseteq[n]$ denote the
$j$th consecutive block
$\beta_j=\{n_1+\cdots+n_{j-1}+1,\dots,n_1+\cdots+n_j\}$. Then:
\begin{enumerate}\alphlabels\tightlist
\item There is a bijection $e:[n]\to I$ with $e(\beta_j)=I_j$ for every $j$.
\item For any such $e$, the map $\Phi:A^{I}\to A^{n}$ defined by
      $\Phi(x)_r:=x(e(r))$ is an isomorphism $\bA^{I}\to\bA^{n}$, and carries
      subuniverses to subuniverses.
\item For every $S\le\bA$ and every $j\in[m]$,
      \[
        \Phi\bigl(\{x\in A^{I}:x(i)\in S\text{ for all }i\in I\setminus I_j\}\bigr)
        =\{\vec z\in A^{n}:z_r\in S\text{ for all }r\in[n]\setminus\beta_j\},
      \]
      and $\Phi(\{x\in A^{I}:x(i)\in S\text{ for all }i\in I\})=S^{n}$.
\end{enumerate}
\end{lemma}
\begin{proof}
(a) Enumerate each $I_j$ as $i_{j,1},\dots,i_{j,n_j}$, which is possible because
$I_j$ is finite and nonempty, and let $e$ send the $q$th element of $\beta_j$ to
$i_{j,q}$. This is a bijection because the $\beta_j$ partition $[n]$ and the
$I_j$ partition $I$.

(b) $\Phi$ is the reindexing along the bijection $e$, so
Lemma~\ref{lem:pp}(e) applies.

(c) $\Phi$ only relabels coordinates: $z_r\in S$ holds for the image exactly
when $x(e(r))\in S$ holds for the source. Since $e(\beta_j)=I_j$, we have
$e([n]\setminus\beta_j)=I\setminus I_j$, which gives the first identity; taking
all coordinates gives the second.
\end{proof}

\begin{remark}[The only relational constructions used]\label{rem:pp-usage}
Parts (a)--(e) of Lemma~\ref{lem:pp}, together with the transport supplied by
Lemma~\ref{lem:grouping}, are all that is needed to see that the relations
built in this document are subuniverses and that they live in the products
their statements claim. They are used in exactly four places: the neighborhoods
$a+R$ of Definition~\ref{def:neighborhood}, the projections and reindexings in
the regrouping Lemma~\ref{lem:essential-groups}, the passage from $\bA^{X}$ to
an enumerated power in Theorem~\ref{thm:relational-absorption}, and the doubled
relation $R'$ of Lemma~\ref{lem:doubling}. No general theory of primitive
positive definability is required.
\end{remark}
...
Let $S\le\bA$, let $n\ge1$ and let $\mathcal P=(P_1,\dots,P_m)$ be a partition
of $[n]$ into $m\ge1$ nonempty blocks. A relation $R\le\bA^{n}$ is
$S$-\emph{essential for} $\mathcal P$ if
\begin{align}
 &R\cap\{\vec x\in A^{n}:x_r\in S\text{ for all }r\in[n]\setminus P_j\}\ne\varnothing
 \quad\text{for every }j\in[m],\label{eq:B1}\\
 &R\cap S^{n}=\varnothing.\label{eq:B2}
\end{align}
When every block is a singleton, so that $m=n$ and $P_j=\{j\}$, this is exactly
$S$-essentiality of arity $n$ in the sense of
...
Let $S\le\bA$, let $m\ge1$ and $n_1,\dots,n_m\ge1$, put $n=n_1+\cdots+n_m$, and
let $\mathcal P$ be the partition of $[n]$ into the $m$ consecutive blocks of
sizes $n_1,\dots,n_m$. If $R\le\bA^{n}$ is $S$-essential for $\mathcal P$ in
the sense of Definition~\ref{def:block-essential}, then there exists an
$S$-essential relation of arity $m$.
\end{lemma}
\begin{proof}
By strong induction on the total arity $n=n_1+\cdots+n_m$, with the number of
blocks $m$ held fixed. Each recursive call below is to a relation with the same
number $m$ of blocks, all of them still of size at least $1$, and with strictly
smaller total arity; these are the only facts about the measure that are used.
Write $P_1,\dots,P_m$ for the consecutive blocks of $\mathcal P$.

If $n_1=\cdots=n_m=1$ then $n=m$, every block is a singleton, and $R$ is
already an $S$-essential relation of arity $m$ by the last sentence of
Definition~\ref{def:block-essential}.

Otherwise some $n_j>1$. Reordering the blocks is a reindexing of $[n]$ that
carries consecutive blocks to consecutive blocks; by Lemma~\ref{lem:pp}(e) it
carries $R$ to a subuniverse of $\bA^{n}$, and it permutes the conditions
\eqref{eq:B1} among themselves while preserving \eqref{eq:B2}. So we may assume
$n_m>1$. Write $n_{<m}=n_1+\cdots+n_{m-1}$, so that $n=n_{<m}+n_m$ and
$P_m=\{n_{<m}+1,\dots,n\}$.

Consider
\[
 R_1:=\pi_{[n-1]}\bigl(R\cap(A^{n-1}\times S)\bigr)\;\le\;\bA^{\,n-1},
\]
a subuniverse by Lemma~\ref{lem:pp}(b),(c), and let $\mathcal P'$ be the
partition of $[n-1]$ into consecutive blocks of sizes
$n_1,\dots,n_{m-1},n_m-1$, all $\ge1$. We test $R_1$ against $\mathcal P'$.

\emph{Condition \eqref{eq:B2}.} If $\vec y\in R_1\cap S^{n-1}$ then
$\vec y=\pi_{[n-1]}(\vec x)$ with $\vec x\in R$ and $x_n\in S$, so
$\vec x\in R\cap S^{n}$, contradicting \eqref{eq:B2} for $R$.

\emph{Condition \eqref{eq:B1} for blocks $j\le m-1$.} The witness $\vec x\in R$
for $P_j$ has every coordinate outside $P_j$ in $S$; in particular $x_n\in S$,
since $n\in P_m$. Hence $\vec x\in R\cap(A^{n-1}\times S)$, and
$\pi_{[n-1]}(\vec x)$ witnesses \eqref{eq:B1} for the $j$th block of
$\mathcal P'$.

\emph{Condition \eqref{eq:B1} for the last block of $\mathcal P'$.} This asks
for $\vec x\in R$ with $x_1,\dots,x_{n_{<m}}\in S$ and $x_n\in S$, the
coordinates $x_{n_{<m}+1},\dots,x_{n-1}$ unconstrained; equivalently, writing
$J:=[n_{<m}]\cup\{n\}$, it asks that
\[
 \pi_{J}(R)\cap S^{\,J}\ne\varnothing. \tag{$\dagger$}
\]

If $(\dagger)$ holds, then $R_1$ is $S$-essential for $\mathcal P'$, of total
arity $n-1<n$, and the inductive hypothesis applies.

If $(\dagger)$ fails, put $R_2':=\pi_{J}(R)\le\bA^{J}$, a subuniverse by
Lemma~\ref{lem:pp}(c), and let $R_2\le\bA^{\,n_{<m}+1}$ be its image under the
reindexing along the order-preserving bijection $J\to[n_{<m}+1]$, which fixes
$1,\dots,n_{<m}$ and sends $n$ to $n_{<m}+1$; this is an isomorphism by
Lemma~\ref{lem:pp}(e). Let $\mathcal P''$ be the partition of $[n_{<m}+1]$ into
consecutive blocks of sizes $n_1,\dots,n_{m-1},1$, all $\ge1$. Condition
\eqref{eq:B2} for $R_2$ is precisely the failure of $(\dagger)$. For
\eqref{eq:B1} at a block $j\le m-1$, project the witness of $R$ for $P_j$: its
coordinates outside $P_j$ lie in $S$, in particular the coordinate $n$. For
\eqref{eq:B1} at the last block $\{n_{<m}+1\}$, project the witness of $R$ for
$P_m$: its first $n_{<m}$ coordinates lie in $S$ and its coordinate $n$ is
unconstrained. Since $n_m>1$ we have $n_{<m}+1<n$, so the inductive hypothesis
applies to $R_2$.
\end{proof}
...
$p_i\!\restriction_X$ lies in
$\{y\in A^{X}:y(\vec x)\in S\text{ for all }\vec x\in X\setminus X_i\}$, so
$\widetilde R$ meets that set.

\emph{Transport to an enumerated power.} The sets $X_1,\dots,X_m$ partition $X$
into nonempty blocks, so Lemma~\ref{lem:grouping} supplies $n:=|X|$, a
block-respecting bijection $e:[n]\to X$ and an isomorphism
$\Phi:\bA^{X}\to\bA^{n}$. Put $R:=\Phi(\widetilde R)\le\bA^{n}$, and let
$\mathcal P$ be the partition of $[n]$ into consecutive blocks of sizes
$n_i:=|X_i|\ge1$. By Lemma~\ref{lem:grouping}(c), the previous paragraph says
exactly that $R$ satisfies \eqref{eq:B1} for $\mathcal P$.

\emph{The all-$S$ box is met.} If $R$ also satisfied \eqref{eq:B2}, it would be
$S$-essential for $\mathcal P$, and Lemma~\ref{lem:essential-groups} would
produce an $S$-essential relation of arity $m$, contrary to hypothesis. So
$R\cap S^{n}\ne\varnothing$, and applying $\Phi^{-1}$ and
Lemma~\ref{lem:grouping}(c) again,
\[
 \widetilde R\cap\{y\in A^{X}:y(\vec x)\in S\text{ for all }\vec x\in X\}\ne\varnothing .
\]
Choose $t\in\Tm(m)$ whose restriction to $X$ witnesses this, so that
...
inferential. Second, several rows therefore cite forward: Remarks
\ref{rem:substitution-usage}, \ref{rem:pp-usage},
\ref{rem:absorption-quantifier}, \ref{rem:taylor-usual},
...
Lemma~\ref{lem:grouping}\newline\emph{Grouping and enumeration} & Lemma~\ref{lem:pp} \\
Remark~\ref{rem:pp-usage}\newline\emph{The only relational constructions used} & Lemma~\ref{lem:pp}, Lemma~\ref{lem:grouping}, Definition~\ref{def:neighborhood}, Lemma~\ref{lem:essential-groups}, Theorem~\ref{thm:relational-absorption} and Lemma~\ref{lem:doubling} \\
...
Theorem~\ref{thm:relational-absorption}\newline\emph{Relational description of absorption, Barto--Kazda \cite{BartoKazda}} & Proposition~\ref{prop:absorption-blocks}, Definition~\ref{def:absorption}, Lemma~\ref{lem:clo-subuniverse}, Lemma~\ref{lem:pp}, Lemma~\ref{lem:grouping}, Lemma~\ref{lem:essential-groups} and Lemma~\ref{lem:preservation} \\
...
Theorem~\ref{thm:center-central}\newline\emph{Zhuk: centrality of the left center} & Lemma~\ref{lem:generation}, Lemma~\ref{lem:sg-terms}, Lemma~\ref{lem:preservation}, Lemma~\ref{lem:substitution-eval}, Lemma~\ref{lem:center-basic} and Convention~\ref{conv:subalgebra} \\
...
\item homomorphisms, the compatibility of images with generation, the five
      relational constructions of Lemma~\ref{lem:pp} --- intersection, box
      intersection, projection, cylinder and reindexing --- and the grouping
      transport of Lemma~\ref{lem:grouping};
...
\item \textbf{Finite enumeration and choice.} A finite set admits an
      enumeration $s_1,\dots,s_N$; a finite set partitioned into consecutive
      blocks admits an enumeration listing the blocks in order
      (Theorem~\ref{thm:center-central}); and a finite nonempty family of
      witnesses admits a choice of one witness from each member, as used when
      selecting the tuples $\vec x^{\,(i)}$ in Lemma~\ref{lem:doubling}.
...
\item \textbf{Elementary inequalities on indices.} For the block bookkeeping of
      Lemma~\ref{lem:essential-groups}: if $n_m>1$ then both $n-1<n$ and
      $n_{<m}+1<n$, where $n=n_{<m}+n_m$.
