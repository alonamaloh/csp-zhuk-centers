...
with open('zhuk_centers.tex', encoding='utf-8') as f:
    src = f.read()

main = src[:src.index('\\appendix')]

env_re = re.compile(
    r'\\begin\{(long)?(theorem|lemma|proposition|corollary|definition|remark|convention)\}'
...
ref_re = re.compile(r'\\ref\{((?:lem|thm|prop|cor|def|rem|conv):[^}]+)\}')
...
rows.append('\\emph{Narrative outside labeled statements}\\newline'
            '(prose of Parts~I--III) & %s \\\\' % fmt_deps(narr_refs))
...
with open('zhuk_centers.tex', 'w', encoding='utf-8') as f:
    f.write(new_src)
